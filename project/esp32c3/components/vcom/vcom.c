...
static inline void vcom_bit_capture(vcom_handle_t *handle)
{
    ...
    if (handle->rx_state == VCOM_RX_STATE_WAIT_START) {
        handle->next_capture_tick = handle->now_tick + handle->sample_count + handle->sample_count / 2;
...
uint8_t vcom_init(vcom_handle_t *handle)
{
    ...
    handle->tx_gpio_write(1);
    /* TODO: check if the interrupt handler is too time-consuming to block next interrupt */
    handle->inited = 1;
